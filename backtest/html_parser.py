...
class EarningsReportParser:
    ...
    def _is_no_stocks_page(self, soup: BeautifulSoup) -> bool:
        ...
        return bool(
            soup.find(
                string=re.compile(r"No.*(?:Qualifying|Earnings).*Stocks.*(?:Found|Available)", re.I)
            )
        )
    ...
    def _extract_candidate(
        self, card, report_date: str, soup: BeautifulSoup
        ...
        parent_section = card.find_parent(
            ["section", "div"], class_=re.compile(r"grade-section|grade-[abcd]")
        )
    ...
    def _extract_grade(self, card, parent_section=None) -> tuple:
        ...
        for badge in card.find_all(
            class_=re.compile(r"grade-badge|rating-badge|large-grade|score-badge")
        ):
        ...
        if parent_section:
            ...
            if header:
                ...
                for badge in header.find_all(
                    class_=re.compile(r"grade-badge|rating-badge|score-badge")
                ):
